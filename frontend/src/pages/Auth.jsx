...
export const AuthPage = ({ mode = "login" }) => {
  ...
  useEffect(() => {
    if (roleParam) {
      setForm((prev) => ({
        ...prev,
        role: roleParam === "patient" ? "user" : roleParam,
      }));
      if (roleParam === "admin") {
  ...
  useEffect(() => {
    if (
      ...
      (form.role === "doctor" || roleParam === "doctor")
    ) {
  ...
  const submit = async (e) => {
    ...
    if (currentMode === "register" && form.role === "admin") {
      showToast(
        "Admin accounts cannot be created through the website. Please contact system administrator.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      if (error.response?.status === 403) {
        ...
        if (errorData.error === "account_pending") {
          showToast(
            "⏳ Your doctor application is under review. Please wait for admin approval before logging in.",
            "error"
          );
        } else if (errorData.error === "account_rejected") {
          showToast(
            "❌ Your doctor application was rejected. Please contact support at support@meditrack.com or submit a new application.",
            "error"
          );
        } else if (errorData.error === "account_not_approved") {
          showToast(
            "🚫 Your doctor account is not approved. Please contact support at support@meditrack.com for assistance.",
            "error"
          );
        } else {
          showToast(
        ...
        showToast(
          "⚠️ An account with this email already exists. Please use a different email or try logging in.",
          "error"
        );
  ...
  const handlePatientRegistrationComplete = async (patientData) => {
    ...
    try {
      ...
      if (result.success) {
        showToast(
          "🎉 Registration completed successfully! Welcome to MediTrack!",
          "success"
        );
      ...
      if (error.response?.status === 409) {
        ...
        showToast(
          "⚠️ This email is already registered. Please use a different email address.",
          "error"
        );
      } else if (error.response?.data?.error === "missing_fields") {
        showToast(
              ...
              >
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-blue-100 text-sm">
                      Track your health progress 24/7
                    </p>
                  </div>
                </div>
          ...
          <div className="w-full lg:w-1/2 bg-white dark:bg-slate-900 flex items-center justify-center p-8">
            ...
            >
              ...
              {roleParam && (
                ...
                >
                  <div
                    className={`w-16 h-16 ${roleInfo.bgColor} rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg`}
                  >
              ...
              >
                <div className="text-center mb-8">
                  ...
                  >
                    {currentMode === "login"
                      ? roleParam
                        ? `Please login to your ${roleInfo.title.toLowerCase()}`
                        : "Please sign in to your account"
                      : roleParam
                      ? `Create your ${roleInfo.title.toLowerCase()}`
                  ...
                  (form.role === "doctor" || roleParam === "doctor") ? (
                    ...
                    >
                      ...
                      <p className="text-slate-600 dark:text-slate-400 mb-6">
                        Please complete the comprehensive doctor registration
                        process with professional credentials, document
                        verification, and availability setup.
                      </p>
                ...
                {roleParam !== "admin" && (
                  <motion.div
                    variants={itemVariants}
                    className="text-center pt-6 border-t border-slate-200 dark:border-slate-600 mt-6"
                  >
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (
                          currentMode === "login" &&
                          (form.role === "user" || form.role === "patient")
                        ) {
